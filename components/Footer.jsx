...
const Footer = () => {
    ...
    const linkSections = [
        ...
        },
        {
            ...
            links: [
                ...
                { text: "contact@shopnest.com", path: 'mailto:contact@shopnest.com', icon: MailIcon },
    ...
    const socialIcons = [
        ...
        { icon: InstagramIcon, link: "https://www.instagram.com/rohan_kashyap.29/" },
        { icon: TwitterIcon, link: "https://twitter.com" },
        { icon: LinkedinIcon, link: "https://www.linkedin.com" },
    ]

    return (
        <footer className="mt-20 bg-slate-100 border-t border-slate-200">
            <div className="max-w-7xl mx-auto px-4 sm:px-6">
                <div className="flex flex-col md:flex-row items-start justify-between gap-10 py-12 sm:py-14 border-b border-slate-500/30 text-slate-500">
                    <div>
                        <Link href="/" className="text-4xl font-semibold text-slate-700">
                            Shop<span className="text-green-600">Nest</span>
                        </Link>
                        <p className="max-w-[410px] mt-6 text-sm">Welcome to ShopNest, your ultimate destination for the latest and smartest gadgets. From smartphones and smartwatches to essential accessories, we bring you the best in innovation — all in one place.</p>
                        <div className="flex items-center gap-3 mt-5">
                            {socialIcons.map((item, i) => (
                                <Link href={item.link} key={i} className="flex items-center justify-center w-10 h-10 bg-white hover:scale-105 hover:border border-slate-300 transition rounded-full">
                                    <item.icon />
                                </Link>
                            ))}
                        </div>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 w-full md:w-auto md:min-w-[45%] gap-8 text-sm">
                ...
                <p className="py-4 text-sm text-slate-500 text-center">
                    Copyright 2026 © ShopNest All Right Reserved.
                </p>
            </div>
        </footer>
    );
